...
//a função do algoritmo está no fim do programa
namespace ArvoreAVL
{
	class ArvoreAVL
	{
		...
		private void Exibir(No no, int nivel)              //O(3n)
		{
			int i;
			...
			Exibir(no.noDireito, nivel + 1);                //O(n)
			Console.WriteLine();

			for (i = 0; i < nivel; i++)                      //O(n)
				Console.Write("    ");

			Console.WriteLine(no.info);

			Exibir(no.noEsquerdo, nivel + 1);               //O(n)
		}
		...
		// em ordem
		private void PercorrerEmOrdem(No no)      //O(3)
		{
			if (no == null)
				return;
		...
		private No Inserir(No no, int x)             //O(12 + log(n))
		{
			if (no == null)
			{
				no = new No(x);
		...
		private No Removeravl(No no, int x)           //O(24 + log(n))
		{
			No ch, s;
